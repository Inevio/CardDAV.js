...
var unescapeStringWithLines = function( str ){
  return ( str || '' ).toString().replace( /\\n/g, '\n' ).replace( /\\;/g, ';' ).replace( /\\,/g, ',' );
};
...
module.exports = {
  ...
  jsonToVCard : function( data ){
    ...
    if( data.org && data.org.office ){
      list.push( 'OFFICE:' + escapeString( data.org.office ) );
    }
  ...
  VCardToJson : function( data ){
    ...
    if( data.note ){
      res.note = unescapeStringWithLines( data.note );
    }

    if( data['x-abshowas'] && data['x-abshowas'].toLowerCase() === 'company' ){
      res.isCompany = true;
    }
